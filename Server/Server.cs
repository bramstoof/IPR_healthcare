        ...
        public Server(string IPaddress, int portNumber) {
            ...
            try {
                ...
                else
                {
                    ...
                    users.Add(new User(username, username, "Root",  time.ToString(), true, 100, UserType.Doctor));
                    username = Encoding.Default.GetString(new SHA256Managed().ComputeHash(Encoding.Default.GetBytes("test")));
                    users.Add(new User(username, username, "Patient",  time.ToString(), true, 100, UserType.Client));
